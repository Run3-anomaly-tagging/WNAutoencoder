...
def main():
    ...
    if args.load_distance_matrix is not None:
        print(f"Loading distance correlation matrix from: {args.load_distance_matrix}")
        distance_matrix = np.load(args.load_distance_matrix)
        ...
        np.save(os.path.join(args.output_dir, "distance_correlation_matrix.npy"), distance_matrix)
    ...
    plot_correlation_matrix(
        distance_matrix, args.output_dir,
