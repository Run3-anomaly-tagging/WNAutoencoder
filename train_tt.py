    ...
    for i_epoch in range(start_epoch, start_epoch + n_epochs):
        ...
        for batch in validation_loader:
            ...
            if(n_batches==0 and plot_config["plot_distributions"]==True and (i_epoch+1 in plot_epochs)):
                ...
                # NEW: Auxiliary feature scatter plot (if auxiliary features present)
                aux_dim = plot_config.get("aux_dim", 0)
                if aux_dim == 2:  # Only plot if we have exactly 2 auxiliary features
                    plot_aux_scatter(data, mcmc, ep_dir, i_epoch+1, aux_dim=2)
...
def main():
    ...
    plot_config = {
        ...
        "rng_seed": 0,
        "plot_dir": PLOT_DIR,
        "aux_dim": 0  # No auxiliary features
    }
    ...
    dataset = JetDataset(DATA_PATH)  # No aux_keys
    ensure_dir(SAVEDIR)
